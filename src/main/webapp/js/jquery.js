...
(function(window, undefined) {
		...
		jQuery.fn = jQuery.prototype = {
			...
			init : function(selector, context, rootjQuery) {
				...
				if( typeof selector === "string") {
					// Are we dealing with HTML string or an ID? 按照html标签获取
			...
			bindReady : function() {
				...
				if(document.addEventListener) {
					...
					if(document.documentElement.doScroll && toplevel) {
						// 在doScrollCheck中，不断的（每隔1ms）执行document.documentElement.doScroll("left")，直到丌抛出异常为止
					    // 这是IE下检测DOM ready的技巧
						doScrollCheck();
					}
				}
	...
	jQuery.extend({
		...
		Deferred : function(func) {
			var doneList = jQuery.Callbacks("once memory"), 
				...
				promise : function(obj) {
					if(obj == null) {
						obj = promise;
					} else { // 在这里,如果promise已经存在(已经调用过.promise()),就不会重新创建了
		...
		when : function(firstParam) {
			...
			if(length > 1) {
				for(; i < length; i++) {
					if(args[i] && args[i].promise && jQuery.isFunction(args[i].promise)) {//如果promise方法存在 在内部实现时会获取obj1.promise()
	...
	jQuery.extend({
		...
		// 页面上jQuery副本的唯一标识
    	// 非数字符号被移除以匹配rinlinejQuery 例如jQuery17208794015547640641
		expando : "jQuery" + (jQuery.fn.jquery + Math.random() ).replace(/\D/g, ""),

		// The following elements throw uncatchable exceptions if you
		// attempt to add expando properties to them.
		// <embed>和<object>标签的区别：两者都是用来播放多媒体文件的对象，
		//object元素用于IE浏览器，embed元素用于非IE浏览器，为了保证兼容性，
		//通常我们同时使用两个元素，浏览器会自动忽略它不支持的标签。同时使用两个元素时，
		//应该把<embed>标签放在<object>标签的内部。
		noData : {
			"embed" : true,
	  ...
	  /**
		...
		removeData : function(elem, name, pvt /* Internal Use Only */ ) {
			...
			internalKey = jQuery.expando, isNode = elem.nodeType,// 内部key
	...
	jQuery.event = {
		...
		trigger : function(event, data, elem, onlyHandlers) {
			...
			for( i = 0; i < eventPath.length && !event.isPropagationStopped(); i++) {
				...
				if(handle && jQuery.acceptData(cur) && handle.apply(cur, data) === false) {//如果是返回 false 就冒泡
	...
	// 创建mouseenter mouseleave事件，将mouseenter修正为mouseover，mouseleave修正为mouseou
	jQuery.each({
	 ...
	 */
	(function() {
		//分割器  http://www.cnblogs.com/nuysoft/archive/2011/11/23/2260877.html
		var chunker = /((?:\((?:\([^()]+\)|[^()]+)+\)|\[(?:\[[^\[\]]*\]|['"][^'"]*['"]|[^\[\]'"]+)+\]|\\.|[^ >+~,(\[\\]+)+|[>+~])(\s*,\s*)?((?:.|\r|\n)*)/g, 
		expando = "sizcache" + (Math.random() + '').replace('.', ''), 
		done = 0, 
		toString = Object.prototype.toString, hasDuplicate = false, 
		baseHasDuplicate = true, rBackslash = /\\/g, 
		rReturn = /\r\n/g, rNonWord = /\W/;
		...
		//1. 分割器 chunker正则
		//2. 块查找 Sizzle.find( expr, context, isXML )块内过滤 Sizzle.filter( expr, set, inplace, not )
		...
		Sizzle.find = function(expr, context, isXML) {
			...
			//Expr.order 依次为 "ID", "CLASS", "NAME", "TAG"
		...
		Sizzle.filter = function(expr, set, inplace, not) {
			var match, anyFound, type, found, item, filter, 
			left, i, pass, old = expr, result = [], 
			curLoop = set, 
				...
				Sizzle = function(query, context, extra, seed) {
					...
					if(!seed && !Sizzle.isXML(context)) {
						...
						if(context.nodeType === 9) {
							...
							try {
								//context.querySelectorAll 是浏览器自带的获取dom对象的方法
	...
	jQuery.extend({
		//修正参数 
		//例如 speed:{width:"70%",opacity:0.4,marginLeft:"0.6in",fontSize:"3em"}
